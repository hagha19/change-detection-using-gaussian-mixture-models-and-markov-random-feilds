function [image,p,t] = freadhdr(fname);

% freadhdr - read BSQ image with ENVI-like header (V. Guissard, Apr 29, 2004)
%
...
% Syntax
%
% image = freadhdr(fname);
% [image,p] = freadhdr(fname);
% [image,p,t] = freadhdr(fname);
%
% INPUT :
%
...
%
% NOTE : freadhdr needs the corresponding image header file generated
%		 automatically by ENVI. The ENVI header file must have the same name
%		 as the ENVI image file + the '.hdr' extension.
%
% Only a very simple 4 line .hdr is needed, the full .hdr file is not needed:
%
% samples = 100
% lines   = 200
% bands   = 3
% data type = 1
%
% the number of spaces before the equals signs is essential
% "data type = 1" means byte data
% "data type = 4" means float data, see below

% Modified (very moderately) by Allan Aasbjerg Nielsen, aa@imm.dtu.dk, 14 Apr 2005
...
while 1
    ...
    switch first
        ...
        case elements(5) % input must be BSQ here
            [f,s] = strtok(second);
            if ~(sum(s==' bsq')==4 | sum(s==' BSQ')==4)
                error('input must be bsq');
            end
    end
end
fclose(rfid); 
...
disp(['Input is (',(num2str(p(1))),' samples) x (', ...
...
%image = reshape(image,[p(1),p(2),p(3)]); % original freadenvi reshape
image = reshape(image,p(1)*p(2),p(3)); % my freadhdr reshape
fclose(fid);
